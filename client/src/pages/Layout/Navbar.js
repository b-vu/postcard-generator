...
function Navbar( {user, setUser} ) {
    ...
    <nav className='flex justify-between items-center py-8 mx-auto h-20 bg-stone-200 font-sans text-black text-xl sticky top-0 z-30 w-full px-2 sm:px-4 shadow-sm' role='navigation'>
        <div className="flex items-center">
            ...
            <h3 className="px-2 text-2xl font-serif font-medium text-black">POSTCARD GENERATOR</h3>
        </div>
        <div className='pr-10 md:block'>
            <button className='p-4 bg-stone-200 rounded-full hover:bg-stone-400 transition duration-300 ease-in-out font-serif'>
                {homeLink}
            </button>
            <button className='p-4 bg-stone-200 rounded-full hover:bg-stone-400 transition duration-300 ease-in-out font-serif'>
                {newCreation}
